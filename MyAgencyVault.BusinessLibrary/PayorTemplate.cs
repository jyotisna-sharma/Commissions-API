...
namespace MyAgencyVault.BusinessLibrary
{
    ...
    public class PayorTemplate
    {
        ...
        /// Created by: Jyotisna
        /// Date: March 23, 2020
        /// Purpose: Get Statement Settings for given payor/template  in import tool settings
        /// </summary>
        /// <param name="PayorID"></param>
        /// <param name="TemplateID"></param>
        /// <returns></returns>
        public static List<ImportToolStatementDataSettings> GetStatementDataList(Guid PayorID, Guid? TemplateID)
        {
            ActionLogger.Logger.WriteLog("GetStatementDataList: processing begins with PayorID" + PayorID, true);

            List<ImportToolStatementDataSettings> statements = new List<ImportToolStatementDataSettings>();
            try
            {
                using (SqlConnection con = new SqlConnection(DBConnection.GetConnectionString()))
                {
                    using (SqlCommand cmd = new SqlCommand("usp_GetStatementDataForImportTool", con))
                    {
                        ...
                        while (reader.Read())
                        {
                            ImportToolStatementDataSettings data = new ImportToolStatementDataSettings();
                            data.TemplateID = reader.IsDBNull("TemplateId") ? Guid.Empty : (Guid)reader["TemplateId"];
                            data.StatementData = reader.IsDBNull("StatementData") ? "" : reader["StatementData"].ToString();
                            data.FixedColLocation = reader.IsDBNull("FixedColsLocation") ? "" : Convert.ToString(reader["FixedColsLocation"]);
                            data.FixedRowLocation = reader.IsDBNull("FixedRowsLocation") ? "" : Convert.ToString(reader["FixedRowsLocation"]);
                            data.PayorID = reader.IsDBNull("PayorId") ? Guid.Empty : (Guid)reader["PayorId"];
                            data.ID = reader.IsDBNull("ID") ? 0 : (int)(reader["ID"]);
                            data.MasterStatementDataID = (int)(reader["MasterStatementDataID"]);
                            data.RelativeColLocation = reader.IsDBNull("RelativeColsLocation") ? "" : Convert.ToString(reader["RelativeColsLocation"]);
                            data.RelativeRowLocation = reader.IsDBNull("RelativeRowsLocation") ? "" : Convert.ToString(reader["RelativeRowsLocation"]);
                            data.BlankFieldsIndicator = Convert.ToString(reader["BlankFieldsIndicator"]);
                            data.IsBlankFieldsIndicatorAvailable = reader.IsDBNull("IsBlankFieldsIndicatorAvailable") ? false : (bool)reader["IsBlankFieldsIndicatorAvailable"];
                            data.RelativeSearch = reader.IsDBNull("RelativeSearch") ? "" : Convert.ToString(reader["RelativeSearch"]);
                            statements.Add(data);
                        }
                        reader.Close();
                    }
            ...
            {
                ActionLogger.Logger.WriteLog("GetStatementDataList:Exception occurs while processing with PayorID-" + PayorID + " " + ex.Message, true);
                throw ex;
            }
        ...
        public static void SaveImportToolStatementData(List<ImportToolStatementDataSettings> data)
        {
            ActionLogger.Logger.WriteLog("SaveImportToolStatementData:processing begins-" + data.ToStringDump(), true);

            try
            {
                foreach (ImportToolStatementDataSettings item in data)
                {
                    AddUpdateImportToolStatementDataSettings(item);
                }
            }
            catch (Exception ex)
            {
                ActionLogger.Logger.WriteLog("SaveImportToolStatementData: Exception occurs while processing-" + ex.Message, true);
                throw ex;
            }
        ...
        /* [DataMember]
         ...
         public List<ImportToolPayorPhrase> CheckAvailability(string strPhrase)
         {
             ...
             {
                 ActionLogger.Logger.WriteLog("CheckAvailability exception" + ex.Message, true);
             }
         ...
         public string Phrase(Guid PayorID, Guid TemplateID)
         {
             ...
             {
                 ActionLogger.Logger.WriteLog("Phrase exceptin: " + ex.Message, true);
             }
             return strPhrase;
         }
         */
        /// <summary>
        /// Used by:Jyotisna
        /// UsedFor:Save the statement data Details
        /// </summary>
        /// <param name="objImportToolStatementDataSettings"></param>
        /// <returns></returns>
        static bool AddUpdateImportToolStatementDataSettings(ImportToolStatementDataSettings objImportToolStatementDataSettings)
        {
            bool bValue = false;
            ActionLogger.Logger.WriteLog("AddUpdateImportToolStatementDataSettings:processing begins-" + objImportToolStatementDataSettings.MasterStatementDataID, true);
            try
            {
                using (DLinq.CommissionDepartmentEntities DataModel = Entity.DataModel)
                {
            ...
            {
                ActionLogger.Logger.WriteLog("AddUpdateImportToolStatementDataSettings:Exception occurs while processing-" + objImportToolStatementDataSettings.MasterStatementDataID + " " + ex.Message, true);
                bValue = false;
                throw ex;
            }
        ...
        /*
         ...
         public void AddUpdatePaymentDataFieldsSetting(ImportToolPaymentDataFieldsSettings objImportToolPaymentDataFieldsSettings)
         {
             try
             {
                 ...
                 using (DLinq.CommissionDepartmentEntities DataModel = Entity.DataModel)
                 {
                     DLinq.ImportToolPaymentDataFieldsSetting temp = DataModel.ImportToolPaymentDataFieldsSettings.FirstOrDefault(s => s.PayorID == objImportToolPaymentDataFieldsSettings.PayorID && s.TemplateID == objImportToolPaymentDataFieldsSettings.TemplateID );
                     if (temp == null)
                     {
