...
namespace MyAgencyVault.BusinessLibrary
{
    ...
    public class PolicyToLearnPost
    {
        ...
        public static void AddUpdatPolicyToLearn(DLinq.Policy _Policy, DEUFields deufields = null)
        {
            ...
            {
                ActionLogger.Logger.WriteLog("AddUpdatPolicyToLearn :" + ex.InnerException.ToString(), true);
            }
        }
        ...
        public static void AddPolicyToLearn(Guid PolicyId, string strRenewal, string strCoverageNickName, string strProductType, Guid? UserID = null, bool updateAll = true)
        {
            try
            {
                using (SqlConnection con = new SqlConnection(DBConnection.GetConnectionString()))
                {
                    string sql = "sp_SaveLearnedFieldsFromPolicy";
                    SqlCommand cmd = new SqlCommand(sql, con);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@PolicyID", PolicyId);
                    cmd.Parameters.AddWithValue("@Renewal", strRenewal);
                    cmd.Parameters.AddWithValue("@Coverage", strCoverageNickName);
                    cmd.Parameters.AddWithValue("@ProductType", strProductType);
                    cmd.Parameters.AddWithValue("@UserID", UserID);
                    cmd.Parameters.AddWithValue("@updateAll", updateAll);
                    con.Open();
                    cmd.ExecuteNonQuery();
                }
            }
            ...
            {
                ActionLogger.Logger.WriteLog("SaveLearnedFields :Error occured with policyId: " + PolicyId + " " + "Exception:" + ex.Message, true);
                throw ex;
            }
        }
        public static void AddPolicyToLearn_old(Guid PolicyId, string strRenewal, string strCoverageNickName,string strProductType)
        {
            using (DLinq.CommissionDepartmentEntities DataModel = Entity.DataModel)
            {
                try
                ...
                {
                    ActionLogger.Logger.WriteLog("Exception AddPolicyToLearn: " + ex.Message, true);
                }
            }
        #endregion
        }
          ...
          public static void AddLearnedAfterImport(Guid PolicyId, string strRenewal, string strCoverageNickName,string strProductType, string ImportID)
        {
            using (DLinq.CommissionDepartmentEntities DataModel = Entity.DataModel)
            {
                ...
                {
                    ActionLogger.Logger.WriteLog("Exception AddlearnedafterImport: " + ex.Message, true);
                }
            }
        }
    }
