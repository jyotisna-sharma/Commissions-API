...
namespace MyAgencyVault.BusinessLibrary
{
    ...
    public class IncomingPament : IEditable<IncomingPament>
    {
        ...
        // ------------------------------------------- Add for Incoming Payment Add--------------------------------
        public static void UpdateInvoiceDate(Guid paymentEntryID, PolicyPaymentEntriesPost paymentEntryData)
        {
            ActionLogger.Logger.WriteLog("UpdateInvoiceDate:paymentEntryID - " + paymentEntryID + ", newInvoice: " + paymentEntryData.InvoiceDate, true);
            try
            {
                ...
                {
                    ActionLogger.Logger.WriteLog("UpdateInvoiceDate:connected - ", true);
                    using (SqlCommand cmd = new SqlCommand("Usp_UpdateInvoiceDate", con))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@newInvoice", Convert.ToDateTime(paymentEntryData.InvoiceDate));
                        cmd.Parameters.AddWithValue("@CommissionPercentage", paymentEntryData.CommissionPercentage);
                        cmd.Parameters.AddWithValue("@NumberOfUnits", paymentEntryData.NumberOfUnits);
                        cmd.Parameters.AddWithValue("@PaymentRecived", paymentEntryData.PaymentRecived);
                        cmd.Parameters.AddWithValue("@DollerPerUnit", paymentEntryData.DollerPerUnit);

                        con.Open();
                        cmd.ExecuteNonQuery();

                        ActionLogger.Logger.WriteLog("UpdateInvoiceDate:exec2 - ", true);
                    }
                }
                /*
                ...
                using (DLinq.CommissionDepartmentEntities DataModel = Entity.DataModel)
                {
                    ...
                    if (_record != null)
                    {
                        ...
                        ActionLogger.Logger.WriteLog(" Success saving invoice date in payment entries: " + _record.InvoiceDate, true);
                        ...
                        if (_deuRecord != null)
                        {
                            ...
                            DataModel.SaveChanges();
                            ActionLogger.Logger.WriteLog(" Success saving invoice date in deu table: " + _deuRecord.InvoiceDate, true);
                        }
                    }
            ...
            {
                ActionLogger.Logger.WriteLog("Exception saving invoice date: " + ex.Message, true);
