...
namespace MyAgencyVault.BusinessLibrary
{
    public class PolicyOutgoingDistribution
    {
        ...
        public static bool CheckIsPaymentFromDEU(Guid OutgoingEntryID)
        {
            ...
            {
                ActionLogger.Logger.WriteLog("CheckIsPaymentFromDEU :" + ex.StackTrace.ToString(), true);
                ActionLogger.Logger.WriteLog("CheckIsPaymentFromDEU :" + ex.InnerException.ToString(), true);
            }
            return result;
        }
        /// <summary>
        ...
        /// <summary>
        /// Use SQL procedure instead of linq 
        /// Created : April 17, 2019
        /// </summary>
        /// <param name="PaymentEntryId"></param>
        /// <returns></returns>
        public static bool CheckIsEntryMarkPaid(Guid PaymentEntryId)
        {
            bool isPaid = false;
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("usp_CheckIsEntryPaid", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@paymentEntryID", PaymentEntryId);
                        con.Open();
                        string result = Convert.ToString(cmd.ExecuteScalar());
                        bool.TryParse(result, out isPaid);
                    }
                }
            }
            catch (Exception ex)
            {
                ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + "CheckIsEntryMarkPaid exception: " + ex.Message, true);
            }
            return isPaid;
        }

        public static bool CheckIsDEUEntryMarkPaid(Guid DEUEntryId)
        {
            bool isPaid = false;
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(DBConnection.GetConnectionString()))
                {
                    using (SqlCommand cmd = new SqlCommand("usp_CheckIsDEUEntryPaid", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
            ...
            {
                ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + "CheckIsDEUEntryMarkPaid exception: " + ex.Message, true);
            }
            return isPaid;
        }
        ...
        {
            ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + "AddUpdateOutgoingPaymentEntry request: " + _PolicyOutgoingDistribution.PaymentEntryId + ", PaidAmount: " + _PolicyOutgoingDistribution.PaidAmount + ", outgoingID: " + _PolicyOutgoingDistribution.OutgoingPaymentId, true);
            bool bValue = true;
            try
            {
                using (DLinq.CommissionDepartmentEntities DataModel = Entity.DataModel)
                {
                    ...
                    if (_ObjPolicyOutgoingPayment == null)
                    {
                        ...
                        ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + "AddUpdateOutgoingPaymentEntry new entry success ", true);
                    }
                    else
                    {
                        ...
                        // _ObjPolicyOutgoingPayment.ReferencedOutgoingScheduleId = _PolicyOutgoingDistribution.ReferencedOutgoingScheduleId;
                        // _ObjPolicyOutgoingPayment.ReferencedOutgoingAdvancedScheduleId = _PolicyOutgoingDistribution.ReferencedOutgoingAdvancedScheduleId;
                        ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + "AddUpdateOutgoingPaymentEntry update success ", true);
                    }
                    DataModel.SaveChanges();
                    bValue = true;
                    ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + "AddUpdateOutgoingPaymentEntry completed  ", true);
                }
            }
            catch (Exception ex)
            {
                ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + "AddUpdateOutgoingPaymentEntry exception: " + ex.Message, true);
                if(ex.InnerException!=null)
                {
                    ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + " AddUpdateOutgoingPaymentEntry inner exception: " + ex.InnerException.Message, true);
                }
                bValue = false;
            }
        ...
        /// ModifiedOn:09-05-2019
        /// </summary>
        /// <param name="EntryId"></param>
        /// <returns></returns>
        public static List<PolicyOutgoingDistribution> GetOutgoingPaymentByPoicyPaymentEntryId(Guid EntryId)
        {
            ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + "GetOutgoingPaymentByPoicyPaymentEntryId:Outgoing Payment request with PaymentEntryId: " + EntryId, true);
            ...
            using (SqlConnection con = new SqlConnection(DBConnection.GetConnectionString()))
            {
                using (SqlCommand cmd = new SqlCommand("Usp_GetOutgoingPayment", con))
                {
                    ...
                    while (reader.Read())
                    {
                        try
                        {
                            ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + "GetOutgoingPaymentByPoicyPaymentEntryId:Reading field values with PaymentEntryId: " + EntryId, true);
                        ...
                        {
                            ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + "GetOutgoingPaymentByPoicyPaymentEntryId:Exception occur while fetching list of OutgoingPayments with PaymentEntryId: " + EntryId + " " + "Exception:" + " " + ex.Message, true);
                        }
        ...
        /// CreatedOn:28-03-2018
        /// Purpose:Getting list of incoming payments 
        /// </summary>
        /// <param name="EntryId"></param>
        /// <returns></returns>
        public static List<PolicyOutgoingPaymentObject> GetOutgoingPaymentList(Guid entryId, ListParams listParams)
        {
            ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + "GetOutgoingPaymentByPoicyPaymentEntryId:Outgoing Payment request with PaymentEntryId: " + entryId, true);
            List<PolicyOutgoingPaymentObject> _PolicyOutgoingDistribution = new List<PolicyOutgoingPaymentObject>();
            DLinq.CommissionDepartmentEntities ctx = new DLinq.CommissionDepartmentEntities(); //create your entity object here
            EntityConnection ec = (EntityConnection)ctx.Connection;
            SqlConnection sc = (SqlConnection)ec.StoreConnection; //get the SQLConnection that your entity object would use
            string adoConnStr = sc.ConnectionString;
            using (SqlConnection con = new SqlConnection(adoConnStr))
            {
                using (SqlCommand cmd = new SqlCommand("Usp_GetOutgoingPayment", con))
                {
                    ...
                    while (reader.Read())
                    {
                        try
                        {
                            PolicyOutgoingPaymentObject objPolicyDetailsData = new PolicyOutgoingPaymentObject();
                            ...
                            {
                                double amt = 0;
                                double.TryParse(Convert.ToString(reader["PaidAmount"]), out amt);
                                objPolicyDetailsData.PaidAmount = "$" + amt.ToString("0.00");
                            }
                            if (!string.IsNullOrEmpty(Convert.ToString(reader["CreatedOn"])))
                            {
                                objPolicyDetailsData.CreatedOn = Convert.ToDateTime(reader["CreatedOn"]);
                            ...
                            if (!string.IsNullOrEmpty(Convert.ToString(reader["Premium"])))
                            {
                                objPolicyDetailsData.OutgoingPremium = Convert.ToString(reader["Premium"]) + "%";
                            }

                            if (!string.IsNullOrEmpty(Convert.ToString(reader["OutGoingPerUnit"])))
                            {
                                double amt = 0;
                                double.TryParse(Convert.ToString(reader["OutGoingPerUnit"]), out amt);
                                objPolicyDetailsData.OutGoingPerUnit = "$" + amt.ToString("0.00");
                            }
                            if (!string.IsNullOrEmpty(Convert.ToString(reader["Payment"])))
                            {
                                objPolicyDetailsData.Payment = Convert.ToString(reader["Payment"]) + "%";
                            }
                            if (!string.IsNullOrEmpty(Convert.ToString(reader["PayeeName"])))
                            {
                                objPolicyDetailsData.PayeeName = Convert.ToString(reader["PayeeName"]);
                            }
                            if (!string.IsNullOrEmpty(Convert.ToString(reader["Status"])))
                            {
                                objPolicyDetailsData.Status = Convert.ToString(reader["Status"]);
                            }
        ...
        /// <param name="PaymentEntryId"></param>
        /// <returns></returns>
        public static bool IfPaymentHasPaidEntry(Guid PaymentEntryId)
        {
            ActionLogger.Logger.WriteLog("IfPaymentHasPaidEntry request: PaymentEntryID - " + PaymentEntryId, true);
            try
            {
                List<PolicyOutgoingDistribution> _PolicyOutgoingDistributionLst
                    = GetOutgoingPaymentByPoicyPaymentEntryId(PaymentEntryId);
                ...
                int paidCount = _PolicyOutgoingDistributionLst.Count(p => p.IsPaid == true);
                ActionLogger.Logger.WriteLog("IfPaymentHasPaidEntry paid count: " + paidCount, true);
                return paidCount > 0;
            }
            catch (Exception ex)
            {
                ActionLogger.Logger.WriteLog("IfPaymentHasPaidEntry exception: PaymentEntryID - " + PaymentEntryId + ", ex: " + ex.Message, true);
                return false;
            }
        }
        ...
        public static void DeleteByPolicyIncomingPaymentId(Guid PaymentEntryId)
        {
            try
            {
                ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + "Outgoing Payment delete request with PaymentEntryId: " + PaymentEntryId, true);
                ...
                ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + "Outgoing Payments deleted with PaymentEntryId: " + PaymentEntryId, true);
            }
            catch (Exception ex)
            {
                ActionLogger.Logger.WriteLog("DeleteByPolicyIncomingPaymentId :" + ex.StackTrace.ToString(), true);
                ActionLogger.Logger.WriteLog("DeleteByPolicyIncomingPaymentId :" + ex.InnerException.ToString(), true);
            }
        ...
        public static string GetOutgoingDeleteEmailContent(Guid OutgoingID)
        {
            ...
            {
                ActionLogger.Logger.WriteLog("GetOutgoingDeleteEmailContent :" + ex.StackTrace.ToString(), true);
                ActionLogger.Logger.WriteLog("GetOutgoingDeleteEmailContent :" + ex.InnerException.ToString(), true);
            }
            return body;
        }
        ...
        {
            ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + "Outgoing Payment Delete request for Outgoing ID: " + OutgoingPaymentid, true);
            try
            {
                using (DLinq.CommissionDepartmentEntities DataModel = Entity.DataModel)
                {
                    ...
                    {
                        // ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + "Outgoing Payment from payment entry: " + _OutgoingDes.PaymentEntryId, true);
                        ...
                        //else
                        {
                            ...
                            DataModel.DeleteObject(_OutgoingDes);
                            DataModel.SaveChanges();
                            ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + "  Outgoing Payment Deleted with Outgoing ID: " + OutgoingPaymentid, true);
            ...
            {
                ActionLogger.Logger.WriteLog("DeleteById :" + ex.StackTrace.ToString(), true);
                ActionLogger.Logger.WriteLog("DeleteById :" + ex.InnerException.ToString(), true);
            }
        }
    }
}
