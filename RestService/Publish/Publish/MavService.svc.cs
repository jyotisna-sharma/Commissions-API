                ...
                if (!string.IsNullOrEmpty(action) && action != "addlog" && (action.Contains("add") || action.Contains("update") || action.Contains("delete") || action.Contains("remove") || action.Contains("link") || action.Contains("post") || action.Contains("save"))) //(!action.Contains("get") && (!action.Contains("load")))) //
                {
                    ...
                    try
                    {
                        ...
                        if (UsernameHeader != null)
                        {
                          ...
                          {
                              ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + " Request received, Action: " + action + ", From user: " + UsernameHeader.UserName, true);
                          });
                            if (string.IsNullOrWhiteSpace(UsernameHeader.UserName) /*&& (action == "deleteclients" || action == "deletepolicy")*/)
                            {

                                MyAgencyVault.BusinessLibrary.MailServerDetail.sendMail("jyotisna@acmeminds.com", "Commission Alert! UserID received blank in request header", mailbody);
                                ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + " AfterReceiveRequest UserName Header is null , action:  " + action, true);
                                request = null;
                                return null;
                            }
                        }
                    ...
                    {
                        ActionLogger.Logger.WriteLog(DateTime.Now.ToString() + " AfterReceiveRequest exception : " + ex.Message, true);
                    }
                }
